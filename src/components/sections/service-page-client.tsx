      ...
      <header className="fixed top-0 z-50 w-full bg-black/90 backdrop-blur-xl border-b border-white/10">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between max-w-7xl">
          ...
          <nav className="hidden md:flex gap-8">
            {navLinks.map((link) => (
              <Link 
                key={link.label} 
                href={link.href} 
                className="text-sm text-neutral-300 hover:text-primary relative group font-medium transition-colors duration-300"
              >
                {link.label}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-primary to-orange-500 group-hover:w-full transition-all duration-300" />
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-4">
            <Button 
              variant="outline" 
              className="hidden md:block border-primary/50 hover:bg-primary/10 hover:scale-105 transition-all duration-300 bg-gradient-to-r from-primary/5 to-transparent backdrop-blur-sm" 
              asChild
            >
        ...
        <nav className={cn(
          ...
          <div className="container mx-auto px-4 py-4 space-y-2">
            ...
            <Button variant="outline" className="w-full mt-4 border-primary/50 hover:bg-primary/10" asChild>
              <Link href="/#contact" onClick={() => setIsMenuOpen(false)}>Book a Meeting</Link>
            </Button>
          </div>
        </nav>
      </header>
